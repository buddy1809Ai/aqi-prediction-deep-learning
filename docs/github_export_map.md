# AQI RESEARCH PROJECT — GITHUB EXPORT MAP

**Total files mapped:** 94
**READY:** 72 | **GIT-LFS:** 20 | **REVIEW:** 1 | **EXCLUDE:** 1

**Status legend:**
- `READY` — commit to GitHub as-is
- `GIT-LFS` — large binary; track with Git LFS (`git lfs track '*.parquet'`)
- `REVIEW` — pilot/superseded; include only if completeness required
- `EXCLUDE` — archived copy; do not commit

---

| Repository File | Source Block | Status | Notes |
|----------------|-------------|--------|-------|
...
## RECOMMENDED .gitattributes (Git LFS)
...
## FILES TO EXCLUDE FROM COMMIT (.gitignore)

```
# Raw data (too large, not redistributable)
CPCB_Data/
# Preprocessed numpy arrays (large; regenerate from pipeline)
outputs/preprocessed/*.npz
# Archived copy artifact
