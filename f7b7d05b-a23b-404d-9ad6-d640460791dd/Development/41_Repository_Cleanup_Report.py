"""
BLOCK 1 — REPOSITORY CLEANUP REPORT
Full 42-block audit: classification, keep/delete decisions, reasons.
NO MODEL TRAINING — pure analysis and documentation.
"""
import os
...
# ── 42-block classification table ────────────────────────────────────────────
blocks = [
    # (block_name, category, decision, reason)
...
import pandas as pd
df = pd.DataFrame(blocks, columns=["Block Name", "Category", "Decision", "Reason"])

keep    = df[df.Decision == "KEEP"]
delete  = df[df.Decision == "DELETE"]
by_cat  = df.groupby(["Category", "Decision"]).size().reset_index(name="Count")

print(SEP)
print("  BLOCK 1 — REPOSITORY CLEANUP AUDIT REPORT")
print(f"  Total blocks: {len(df)}  |  KEEP: {len(keep)}  |  DELETE: {len(delete)}")
print(SEP)

cat_order = ["Core Pipeline","Research Analysis","Deployment","Temporary Diagnostic","Duplicate","Obsolete"]
for cat in cat_order:
    sub = df[df.Category == cat]
    if sub.empty:
        continue
    print(f"\n{'─'*60}")
    print(f"  {cat.upper()}  ({len(sub)} blocks)")
    print(f"{'─'*60}")
    for _, r in sub.iterrows():
        sym = "✓ KEEP  " if r.Decision == "KEEP" else "✗ DELETE"
        print(f"  [{sym}]  {r['Block Name']}")
        print(f"           {r['Reason']}")

print(f"\n{SEP}")
print("  BLOCKS MARKED FOR DELETION (5 total)")
print(SEP)
for _, r in delete.iterrows():
...
md_lines = [
    ...
    f"**Total blocks:** {len(df)} | **Keep:** {len(keep)} | **Delete:** {len(delete)}\n",
    "\n## Block Classification Table\n",
    "| Block Name | Category | Decision | Reason |",
    "|---|---|---|---|",
]
for _, r in df.iterrows():
    md_lines.append(f"| {r['Block Name']} | {r['Category']} | {r['Decision']} | {r['Reason']} |")
...
for _, r in delete.iterrows():
    md_lines.append(f"| {r['Block Name']} | {r['Reason']} |")
...
for cat in cat_order:
    sub = df[df.Category == cat]
    k = len(sub[sub.Decision == "KEEP"])
    d = len(sub[sub.Decision == "DELETE"])
    if not sub.empty:
        md_lines.append(f"| {cat} | {k} | {d} |")

md_lines.append("\n## Recommended Pipeline Sequence\n```\n" + pipeline_order + "\n```\n")

report_path = OUT / "repository_cleanup_report.md"
with open(report_path, "w") as f:
    f.write("\n".join(md_lines))

df.to_csv(OUT / "block_classification.csv", index=False)

print(f"✓ Saved → {report_path}")
print(f"✓ Saved → outputs/block_classification.csv")
print(f"\n  SUMMARY: {len(keep)} blocks to keep, {len(delete)} blocks to delete")
