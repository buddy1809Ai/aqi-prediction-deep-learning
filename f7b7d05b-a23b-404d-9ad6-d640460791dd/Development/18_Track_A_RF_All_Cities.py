...
print(f"\n{SEP}\n  BLOCK 2 — RANDOM FOREST — All Cities ({len(parquets)} found)\n{SEP}")
...
for pq in parquets:
    ...
    results.append({
        'city': city, 'model': 'RandomForest', 'track': 'A',
...
if len(track_a_rf_df) > 0:
    print(f"  Cities done : {len(track_a_rf_df)}")
    print(f"  Avg R²      : {track_a_rf_df['r2'].mean():.4f}")
    print(f"  Avg MAE     : {track_a_rf_df['mae'].mean():.2f}")
    print(f"  Avg RMSE    : {track_a_rf_df['rmse'].mean():.2f}")
    print(f"  Best city   : {track_a_rf_df.loc[track_a_rf_df['r2'].idxmax(),'city']}  "
          f"(R²={track_a_rf_df['r2'].max():.4f})")
    print(f"  Worst city  : {track_a_rf_df.loc[track_a_rf_df['r2'].idxmin(),'city']}  "
          f"(R²={track_a_rf_df['r2'].min():.4f})")
print(f"  Saved → {CKPT}\n{SEP}")
