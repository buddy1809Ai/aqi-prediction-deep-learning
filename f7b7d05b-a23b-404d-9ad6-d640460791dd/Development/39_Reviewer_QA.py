...
# ─── SECTION 1: LEAKAGE (Q1–Q7) ─────────────────────────────────────────────
{
"q": "Q1. The R² values for Track A (0.97–0.99) appear suspiciously high. Is there target leakage?",
"a": """No. The high R² in Track A (AQI Estimation) is scientifically expected and does NOT constitute
...
{
"q": "Q2. Did you include AQI lag features (AQI at t-1h, t-6h) in your models?",
"a": """No. All AQI-derived features (26 total: AQI_lag_1h through AQI_lag_48h, AQI rolling means,
...
{
"q": "Q3. How do you confirm that scalers were not fitted on test data?",
"a": """Chronological splitting was applied: 70% train, 15% validation, 15% test (no shuffling).
...
{
"q": "Q4. Is it possible that the sequence generation for LSTM introduced future leakage?",
"a": """No. Sequences of length 24 were constructed strictly in chronological order from the
...
{
"q": "Q5. Were forecast horizon targets (t+1h, t+6h, t+24h) correctly shifted?",
"a": """Yes. For each horizon h ∈ {1, 6, 24}, the target was created via:
...
{
"q": "Q6. Track A uses same-timestamp pollutants. Is this truly different from using AQI itself?",
"a": """Yes — with an important nuance. Raw pollutant concentrations (PM2.5 in µg/m³, etc.) are
...
{
"q": "Q7. Track B allows pollutant lag features. Do those indirectly contain AQI information?",
"a": """Pollutant lags (PM2.5 at t-1h, t-6h, etc.) are chemically meaningful temporal observations
...
# ─── SECTION 2: AQI FORMULA (Q8–Q12) ──────────────────────────────────────
{
"q": "Q8. If AQI is deterministically computable from pollutants, why train a model at all?",
"a": """Two reasons. First, in real-world CPCB datasets there are sensor dropouts, calibration
...
{
"q": "Q9. Why does LSTM underperform (R²=0.64) so severely on Track A vs. tree models (R²≈0.99)?",
"a": """AQI Estimation is structurally a TABULAR REGRESSION problem: the output at time t depends
...
{
"q": "Q10. Should Track A results be published, given they essentially reproduce a known formula?",
"a": """Yes, for several reasons: (1) The study demonstrates that ML can reliably reconstruct
...
{
"q": "Q11. What is the practical difference between AQI Estimation and AQI Forecasting?",
"a": """AQI Estimation (Track A): Inputs include same-timestamp pollutant sensor readings.
...
{
"q": "Q12. Your study covers 18 cities. Is the model generalizable to unseen Indian cities?",
"a": """Partially. The study uses city-specific models (one GradBoost per city), which cannot
...
# ─── SECTION 3: DEEP LEARNING (Q13–Q18) ───────────────────────────────────
{
"q": "Q13. Why does deep learning consistently underperform classical ML in this study?",
"a": """Three converging reasons: (1) ARCHITECTURE MISMATCH — LSTM/BiLSTM are sequence models.
...
{
"q": "Q14. Why does CNN-BiLSTM perform worst (Track A R²=0.28, Track B R²=-0.41)?",
"a": """CNN-BiLSTM applies Conv1D + MaxPooling1D before the BiLSTM. MaxPooling(2) halves the
...
{
"q": "Q15. Did you perform hyperparameter tuning for LSTM?",
"a": """Hyperparameters were selected based on domain conventions for hourly air quality
...
{
"q": "Q16. Should LSTM be eliminated from the final recommendation given its poor Track A performance?",
"a": """For Track A (Estimation): Yes — LSTM is not recommended for concurrent tabular regression.
...
{
"q": "Q17. Is BiLSTM better than LSTM for AQI forecasting?",
"a": """Marginally, in Track B at t+1h (BiLSTM=0.34 vs LSTM=0.43, with LSTM actually better
...
{
"q": "Q18. Why is deep learning DL avg R² only 0.048 in Track B?",
"a": """The Track B DL average R²=0.048 is heavily dragged down by CNN-BiLSTM (avg R²=-0.41),
...
# ─── SECTION 4: METHODOLOGY (Q19–Q23) ─────────────────────────────────────
{
"q": "Q19. Why did you choose Gradient Boosting over XGBoost, given XGBoost's popularity?",
"a": """GradBoost (sklearn) outperforms XGBoost in this study for Track A (R²=0.9906 vs 0.9856)
...
{
"q": "Q20. Why did you use 70/15/15 train/val/test splits instead of k-fold cross-validation?",
"a": """Time-series data requires chronological (non-shuffled) splitting to prevent future
...
{
"q": "Q21. Navi Mumbai was excluded. Why? How does this affect generalizability?",
"a": """Navi Mumbai's cleaned parquet contained only 48 rows — insufficient for training,
...
{
"q": "Q22. Why did you use median imputation + ffill/bfill instead of more sophisticated imputation?",
"a": """The recovery strategy prioritizes maximum row preservation over optimal imputation quality,
...
{
"q": "Q23. How did you prevent data from different cities contaminating each other?",
"a": """City-specific modeling was used throughout. Each city has its own cleaned parquet,
...
# ─── SECTION 5: LIMITATIONS (Q24–Q27) ─────────────────────────────────────
{
"q": "Q24. What are the main limitations of this study?",
"a": """1. City-specific models: No global or transfer learning — new cities require retraining.
...
{
"q": "Q25. How does model performance degrade for cities with high missing data (Vapi, Jodhpur)?",
"a": """Cities with >40% original missing data (Vapi, Jodhpur) show: (1) GradBoost Track A
...
{
"q": "Q26. The CNN-BiLSTM produces negative R² in Track B. Should it be excluded from publication?",
"a": """No — the negative R² results are scientifically informative and should be reported.
...
{
"q": "Q27. What is the statistical significance of the R² differences between GradBoost and RF?",
"a": """The GradBoost vs. RandomForest gap in Track A is Δ R²=0.0032 — very small. Across 18
...
# ─── SECTION 6: REPRODUCIBILITY (Q28–Q30) ─────────────────────────────────
{
"q": "Q28. Is this study fully reproducible? What would a reviewer need to replicate it?",
"a": """Yes. The full pipeline is implemented across 36 Zerve notebook blocks in strict
...
{
"q": "Q29. Did you test for statistical stationarity of the time series before modeling?",
"a": """Stationarity testing (ADF test) was not performed in this study. The feature engineering
...
{
"q": "Q30. What is the recommended journal and conference venue for submission?",
"a": """Based on the dual-track framework, multi-city scope, and environmental ML focus:
...
# Print summary
print("=" * 70)
print("  BLOCK 5 — REVIEWER Q&A GENERATION")
print("  30 Questions across 6 categories — no training")
print("=" * 70)
categories = [
    "Leakage (Q1–Q7)", "AQI Formula (Q8–Q12)", "Deep Learning (Q13–Q18)",
    "Methodology (Q19–Q23)", "Limitations (Q24–Q27)", "Reproducibility (Q28–Q30)"
]
for cat in categories:
    print(f"  ✓ {cat}")

print(f"\n  Total Q&A pairs: {len(reviewer_qa)}")
...
sections = {
    "Section 1 — Data Leakage": range(0, 7),
    "Section 2 — AQI Formula & Task Definition": range(7, 12),
    "Section 3 — Deep Learning Performance": range(12, 18),
    "Section 4 — Methodology & Design Choices": range(18, 23),
    "Section 5 — Limitations": range(23, 27),
    "Section 6 — Reproducibility & Publication": range(27, 30),
}

for section_title, idx_range in sections.items():
    md_lines.append(f"## {section_title}")
    md_lines.append("")
    for i in idx_range:
        if i >= len(reviewer_qa): break
        qa = reviewer_qa[i]
        md_lines.append(f"### {qa['q']}")
        md_lines.append("")
        md_lines.append(qa['a'].strip())
        md_lines.append("")
        md_lines.append("---")
        md_lines.append("")
...
with open(OUT / "reviewer_qa.md", "w") as f:
    f.write("\n".join(md_lines))
print(f"\n  ✓ Saved: outputs/reviewer_qa.md")
print(f"  File size: {(OUT / 'reviewer_qa.md').stat().st_size / 1024:.1f} KB")
