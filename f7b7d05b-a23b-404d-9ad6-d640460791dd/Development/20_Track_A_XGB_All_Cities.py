...
print(f"\n{SEP}\n  BLOCK 4 — XGBOOST — All Cities ({len(parquets)} found)\n{SEP}")
...
for pq in parquets:
    ...
    # Sanitize feature names (XGBoost dislikes special chars)
    safe_names = [f"f{i}" for i in range(len(feat_cols))]

    t0  = time.time()
    mdl = XGBRegressor(n_estimators=300, max_depth=6, learning_rate=0.05,
    ...
    results.append({
        'city': city, 'model': 'XGBoost', 'track': 'A',
...
print(f"\n{SEP}\n  XGBOOST SUMMARY")
if len(track_a_xgb_df) > 0:
    print(f"  Cities done : {len(track_a_xgb_df)}")
    print(f"  Avg R²      : {track_a_xgb_df['r2'].mean():.4f}")
    print(f"  Avg MAE     : {track_a_xgb_df['mae'].mean():.2f}")
    print(f"  Avg RMSE    : {track_a_xgb_df['rmse'].mean():.2f}")
    print(f"  Best city   : {track_a_xgb_df.loc[track_a_xgb_df['r2'].idxmax(),'city']}  "
          f"(R²={track_a_xgb_df['r2'].max():.4f})")
    print(f"  Worst city  : {track_a_xgb_df.loc[track_a_xgb_df['r2'].idxmin(),'city']}  "
          f"(R²={track_a_xgb_df['r2'].min():.4f})")
print(f"  Saved → {CKPT}\n{SEP}")
