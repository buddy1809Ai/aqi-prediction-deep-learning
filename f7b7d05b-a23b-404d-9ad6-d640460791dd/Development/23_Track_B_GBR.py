...
print(f"\n{SEP}\n  TRACK B — GRADIENT BOOSTING  (18 cities × 3 horizons)\n"
...
for pq in parquets:
    ...
    for h in HORIZONS:
        if (city, h) in done_keys:
            print(f"    t+{h:02d}h  GBR  ✓ skip"); continue
        ...
        if len(y_te) < 100:
            print(f"    t+{h:02d}h ✗ insufficient test rows"); continue

        mdl = GradientBoostingRegressor(n_estimators=150, max_depth=5,
                                        learning_rate=0.05, subsample=0.8,
                                        random_state=SEED)
        t0 = time.time(); mdl.fit(X_tr, y_tr); tr_t = time.time()-t0
        t1 = time.time(); yp = mdl.predict(X_te); inf_t = time.time()-t1

        r2v  = float(r2_score(y_te, yp))
        maev = float(mean_absolute_error(y_te, yp))
        rv   = rmse(y_te, yp)

        results.append({'city':city,'horizon':h,'model':'GradientBoosting','track':'B',
        ...
        print(f"    t+{h:02d}h  GBR  R²={r2v:.4f}  MAE={maev:.1f}  RMSE={rv:.1f}  t={tr_t:.1f}s")

track_b_gbr_df = pd.DataFrame(results)
track_b_gbr_df.to_csv(CKPT, index=False)

print(f"\n{SEP}\n  TRACK B — GRADIENT BOOSTING SUMMARY\n{SEP}")
for h in HORIZONS:
    sub = track_b_gbr_df[track_b_gbr_df['horizon']==h]
