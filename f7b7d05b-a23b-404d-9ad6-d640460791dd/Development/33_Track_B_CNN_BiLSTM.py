...
# ── Zerve/h5py TF import fix ─────────────────────────────────────────────────
# h5py (pulled in by TF) calls platform._Processor → subprocess.check_output
# → subprocess.Popen → io.open(int_fd, 'rb').  Zerve's io.open wrapper only
# accepts path strings and raises TypeError on int file-descriptors.
# The C-level _io.open handles both paths AND int fds natively.
# We install a shim BEFORE importing TF, then restore after.
# NOTE: os.fdopen (frozen in Py 3.11) itself calls io.open internally, so we
# must NOT call os.fdopen in the shim — use _real_io_open directly for int fds.
_zerve_io_open = _io_mod.open
_real_io_open  = _c_io.open   # true C-level open — handles int fds natively

def _fd_safe_io_open(file, mode='r', *args, **kwargs):
    if isinstance(file, int):
        return _real_io_open(file, mode, *args, **kwargs)   # int fd → C-level
    return _zerve_io_open(file, mode, *args, **kwargs)      # path → Zerve open

_io_mod.open = _fd_safe_io_open   # patch before TF import
...
_io_mod.open = _zerve_io_open     # restore normal Zerve open after TF loaded
...
if CKPT.exists():
    ...
    print(f"Checkpoint: {len(done_keys)} city×horizon rows already done")
else:
    results = []

parquets = sorted(REC_DIR.glob("*_recovered.parquet"))
print(SEP)
print(f"  TRACK B — CNN-BiLSTM  (18 cities × 3 horizons)")
print(f"  seq_len={SEQ}  |  Features: lags + rolling + met + time")
print(f"  EXCLUDED: same-t pollutants, all AQI-derived features")
print(SEP)
...
for pq in parquets:
    ...
    for h in HORIZONS:
        ...
        Xs_te, _ys   = make_sequences(Xte_s, np.zeros(len(Xte_s)), SEQ)
        yte_aligned  = yte[SEQ:]   # align with sequence output

        if len(Xs_te) < 50:
            print(f"    t+{h:02d}h ✗ insufficient sequences")
            continue
        ...
        print(f"    t+{h:02d}h  CNN-BiLSTM  R²={r2v:.4f}  MAE={maev:.1f}  "
              f"RMSE={rv:.1f}  t={tr_t:.1f}s")

        results.append({"city": c, "horizon": h, "model": "CNN-BiLSTM", "track": "B",
                         "n_train": len(ys_tr), "n_test": len(yte_aligned),
...
print()
print(SEP)
print("  TRACK B — CNN-BiLSTM SUMMARY")
print(SEP)
for h in HORIZONS:
    sub = track_b_cnn_bilstm_df[track_b_cnn_bilstm_df["horizon"] == h]
    if len(sub) == 0: continue
    print(f"  Horizon t+{h:02d}h  |  cities={len(sub)}")
    ...
    print(f"    Best : {best['city']}  R²={best['r2']:.4f}")
    print(f"    Worst: {worst['city']}  R²={worst['r2']:.4f}")
print(f"\n  Saved → {CKPT}")
print(SEP)
